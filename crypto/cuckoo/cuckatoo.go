...
func Sipnode(h *siphash.SipHash,edge ,uorv uint64,shift bool) uint64 {
...
func VerifyCuckatoo(sipkey []byte, nonces []uint32,edgeBits uint) error {
	...
	for n := 0; n < ProofSize; n++ {
		...
		uvs[2 * n] = Sipnode(sip,uint64(nonces[n]),uint64(0),false)
		uvs[2 * n+1] = Sipnode(sip,uint64(nonces[n]),uint64(1),false)
		xor0 ^= uvs[2 * n]
		xor1 ^= uvs[2 * n+1]
	}
	if xor0 != 0 {
